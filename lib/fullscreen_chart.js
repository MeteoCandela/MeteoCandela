...
export function initChartFullscreen(canvasIds = []) {
  ...
  function openForCanvas(canvas) {
    ...
    // Títol "net": el guardem a data-fs-title (veure més avall),
    // així evitem que aparegui el símbol ⛶ dins el títol del modal.
    const card = canvas.closest(".card");
    const h2 = card ? card.querySelector("h2") : null;

    const titleClean =
      h2?.dataset?.fsTitle ||
      (h2?.textContent || "").replace(/\s*⛶\s*$/, "").trim() ||
      "Gràfica";
  ...
  // Events de tancar
  if (fsClose && !fsClose.__fsBound) {
    fsClose.addEventListener("click", closeModal);
    fsClose.__fsBound = true;
  }
  ...
  for (const id of canvasIds) {
    ...
    // Guarda el títol "net" (abans d'afegir el botó!)
    if (!h2.dataset.fsTitle) {
      const clean = (h2.textContent || "").replace(/\s*⛶\s*$/, "").trim();
      h2.dataset.fsTitle = clean;
    }
