...
export function renderSunSub() {
  // HOME: chipSun
  const elChip = $("chipSun");
  // Compat: antics layouts
  const elOld = $("sunSub");

  const st = getSunTimesToday();
  if (!st) {
    if (elChip) elChip.textContent = "—";
    if (elOld) elOld.textContent = "Sol: —";
    return;
  }

  const txt = `Sortida ${fmtHM(st.sunrise)}h · Posta ${fmtHM(st.sunset)}h`;

  if (elChip) elChip.textContent = txt;
  if (elOld) elOld.textContent = `Sol: ${txt}`;
}
