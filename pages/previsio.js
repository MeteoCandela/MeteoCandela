...
// -------- LocalStorage (més robust: per ID de zona) --------
const LS_ZONE_ID = "meteovalls:fx_zone_id";
const LS_MUNI_GLOBAL = "meteovalls:muni_id"; // compat vell
const LS_MUNI_BY_ZONEID_PREFIX = "meteovalls:muni_id:zone:";

// -------- IDs --------
const VALLS_ID = "43161";

// -------- Defaults per zona CANÒNICA --------
const DEFAULT_BY_CANON_LABEL = {
  "Municipis Alt Camp": VALLS_ID,
  "Capitals província Tarragona": "43148",
  "Capitals província Barcelona": "08019",
  "Capitals província Girona": "17079",
  "Capitals província Lleida": "25120",
};

function muniKeyForZoneId(zoneId){
  return `${LS_MUNI_BY_ZONEID_PREFIX}${String(zoneId || "")}`;
}
...
function getSelectedZoneLabel(){
  return getSelectedText("zoneSelect").trim();
}
...
// -------- Normalització zones (aquí està la clau) --------
function canonZoneLabel(label){
  const s = String(label || "").toLowerCase();

  if (s.includes("alt camp")) return "Municipis Alt Camp";
  if (s.includes("tarragona")) return "Capitals província Tarragona";
  if (s.includes("barcelona")) return "Capitals província Barcelona";
  if (s.includes("girona")) return "Capitals província Girona";
  if (s.includes("lleida")) return "Capitals província Lleida";
...
function normalizeZones(raw){
  ...
  return arr
    .map(z => ({
      id: String(z.id || z.zone_id || z.key || canonZoneLabel(z.label)), // fem un id si no en ve
      label: canonZoneLabel(z.label),
      items: (Array.isArray(z.items) ? z.items : []).map(it => ({
        id: String(it.id),
        name: String(it.name),
      })),
    }))
    .filter(z => z.label && z.items.length);
}

function optionLabel(m){
  const name = String(m?.name || "");
  if (String(m?.id) === VALLS_ID) return name.startsWith("★") ? name : `★ ${name}`;
  return name;
}
...
export function initPrevisio() {
  ...
  let zones = []; // [{id,label,items:[{id,name}]}]

  function getZoneById(zoneId){
    const zid = String(zoneId || "");
    return zones.find(z => String(z.id) === zid) || null;
  }
  function findZoneIdContainingMuni(muniId){
    const id = String(muniId || "");
    if (!id) return null;
    for (const z of zones) {
      if ((z.items || []).some(it => String(it.id) === id)) return String(z.id);
    }
    return null;
  }

  function populateZoneSelect(startZoneId){
    if (!zoneSel) return;
    zoneSel.innerHTML = zones.map(z => `<option value="${String(z.id)}">${String(z.label)}</option>`).join("");
    const exists = startZoneId && zones.some(z => String(z.id) === String(startZoneId));
    zoneSel.value = exists ? String(startZoneId) : (zones[0] ? String(zones[0].id) : "");
  }

  function populateMuniSelect(zoneId, muniIdToSelect){
    if (!muniSel) return;
    const z = getZoneById(zoneId);
    const items = Array.isArray(z?.items) ? z.items : [];

    muniSel.innerHTML = items.map(m => `<option value="${String(m.id)}">${optionLabel(m)}</option>`).join("");
    ...
    const canonLabel = z?.label ? String(z.label) : "";
    const def = DEFAULT_BY_CANON_LABEL[canonLabel] || null;
    if (def && items.some(m => String(m.id) === String(def))) {
      muniSel.value = String(def);
      return;
    }

    if (items[0]) muniSel.value = String(items[0].id);
  }
  ...
  async function initSelectors(){
    // 1) carrega municipis (IMPORTANT: pot venir com zones o groups)
    ...
    const raw = Array.isArray(cfg?.zones) ? cfg.zones : (Array.isArray(cfg?.groups) ? cfg.groups : []);
    zones = normalizeZones(raw);

    if (!zones.length) {
      throw new Error("Config municipis buida: /api/municipis no retorna zones/groups vàlids.");
    }

    // 2) zona inicial (per ID)
    const savedZoneId = localStorage.getItem(LS_ZONE_ID);
    const savedGlobalMuni = localStorage.getItem(LS_MUNI_GLOBAL);
    const inferredZoneId = findZoneIdContainingMuni(savedGlobalMuni);

    const startZoneId =
      (savedZoneId && getZoneById(savedZoneId)) ? savedZoneId :
      (inferredZoneId && getZoneById(inferredZoneId)) ? inferredZoneId :
      String(zones[0].id);

    populateZoneSelect(startZoneId);

    // 3) municipi inicial per zona (per ID)
    const currentZoneId = zoneSel ? String(zoneSel.value || startZoneId) : startZoneId;
    const savedMuniForZone = localStorage.getItem(muniKeyForZoneId(currentZoneId));

    populateMuniSelect(currentZoneId, savedMuniForZone || savedGlobalMuni);

    // 4) render inicial
    setHeaderPlace();
    await loadAndRender(muniSel ? String(muniSel.value || VALLS_ID) : VALLS_ID);

    // 5) listeners
    if (zoneSel) {
      zoneSel.addEventListener("change", async () => {
        const zid = String(zoneSel.value || "");
        localStorage.setItem(LS_ZONE_ID, zid);

        const lastMuni = localStorage.getItem(muniKeyForZoneId(zid));
        populateMuniSelect(zid, lastMuni);
        ...
        if (muniId) {
          ...
          localStorage.setItem(muniKeyForZoneId(zid), muniId);
    ...
    if (muniSel) {
      muniSel.addEventListener("change", async () => {
        const zid = zoneSel ? String(zoneSel.value || "") : "";
        const id = String(muniSel.value || "");
        if (!id) return;

        localStorage.setItem(LS_MUNI_GLOBAL, id);
        if (zid) localStorage.setItem(muniKeyForZoneId(zid), id);
  ...
  initSelectors().catch(async (e) => {
    console.error(e);
    if (status) status.textContent = `Error inicialitzant selectors: ${e?.message || e}`;

    if (zoneSel) zoneSel.innerHTML = `<option value="altcamp">Municipis Alt Camp</option>`;
    if (muniSel) muniSel.innerHTML = `<option value="${VALLS_ID}">★ Valls</option>`;
