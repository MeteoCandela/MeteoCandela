...
const CALENDARI_DATA = {
  ...
  crops: [
    ...
    {id:"patata_tardana", nom:"Patata tardana", familia:"arrels", categoria:"tuber", tipus_implantacio:"directa", calendaris:{sembra_interior:[], sembla_exterior:[["Ag1","Ag2"]], trasplantament:[], collita:[["N1","N2"]]}, tags:["RG"], notes:"Cicle tardà; necessita reg i control del míldiu.", proteccio_recomanada:false, ordre:35},
    ...
    {id:"farigola", nom:"Farigola (plantació)", familia:"aromatiques", categoria:"aromàtica", tipus_implantacio:"plantació", calendaris:{sembra_interior:[], sembra_exterior:[["Mç1","Mg2"]], trasplantament:[], collita:[["Mg1","O2"]]}, tags:[], notes:"Perenne; collita principal a primavera-estiu.", proteccio_recomanada:false, ordre:48},
...
function cropMatches(crop) {
  ...
  const text = [crop.nom, crop.notes, familyName, crop.categoria, crop.tipus_implantacio, crop.tags.join(' ')].join(' ').toLowerCase();
  return (!mcState.q || text.includes(mcState.q.toLowerCase()))
...
function renderMeta() {
  ...
  $('mcLegendTags').innerHTML = Object.entries(CALENDARI_DATA.meta.llegenda_tags)
    .map(([k, v]) => `<div class="legend-chip"><b>${k}</b> ${escapeHtml(v)}</div>`)
    .join('');
}

function fillFilters() {
  $('mcFamily').innerHTML = `<option value="all">Totes les famílies</option>` + CALENDARI_DATA.families.map(f => `<option value="${f.id}">${f.nom}</option>`).join('');
  $('mcTag').innerHTML = `<option value="all">Totes les etiquetes</option>` + Object.keys(CALENDARI_DATA.meta.llegenda_tags).map(k => `<option value="${k}">${k}</option>`).join('');
  $('mcPhase').innerHTML = `<option value="all">Qualsevol fase</option>` + MC_PHASES.map(p => `<option value="${p.key}">${p.long}</option>`).join('');
  $('mcHalf').innerHTML = `<option value="all">Tot l'any</option>` + CALENDARI_DATA.meta.codi_quinzenes.map(k => `<option value="${k}">${k}</option>`).join('');
}
...
function renderStats(crops) {
  ...
  const waterDemand = crops.filter(c => c.tags.includes('RG')).length;

  $('mcStats').innerHTML = [
    stat(crops.length, 'cultius visibles'),
    stat(visibleFamilies, 'famílies representades'),
    stat(withProtection, 'amb protecció recomanada'),
    stat(waterDemand, 'amb reg exigent')
  ].join('');
}
...
function cropCard(crop) {
  return `
    <article class="card crop-card" style="--mc-fam-color:${famColor(crop.familia)}">
      <div class="crop-top">
        <div>
          <div class="crop-name">${escapeHtml(crop.nom)}</div>
          <div class="crop-sub">${escapeHtml(MC_FAMILY_MAP[crop.familia].nom)} · ${escapeHtml(crop.tipus_implantacio)}</div>
        </div>
        <span class="cat-chip">${escapeHtml(crop.categoria)}</span>
      </div>
      <div class="crop-tags">${crop.tags.map(t => `<span class="tag" data-tag="${t}">${t}</span>`).join('') || '<span class="tag">sense etiqueta</span>'}</div>
      <div class="phase-list">
        ${MC_PHASES.map(p => `
          <div class="phase-row">
            <b>${p.label}</b>
            <div class="phase-badges">${phaseRanges(crop, p.key).map(r => `<span class="phase-chip ${p.cls}">${r}</span>`).join('') || '<span class="small">—</span>'}</div>
          </div>
        `).join('')}
      </div>
      <div class="crop-notes ${mcState.showNotes ? '' : 'hidden'}">${escapeHtml(crop.notes)}</div>
    </article>
  `;
}

function renderCards(crops) {
  $('mcViewCards').innerHTML = `<div class="crop-grid">${crops.map(cropCard).join('')}</div>`;
}

function renderFamilies(crops) {
  const groups = CALENDARI_DATA.families.map(f => ({ ...f, items: crops.filter(c => c.familia === f.id) })).filter(g => g.items.length);
  $('mcViewFamilies').innerHTML = groups.map(g => `
    <section class="family-block" style="--mc-fam-color:${famColor(g.id)}">
      <div class="family-head">
        <div class="family-title"><span class="family-dot"></span><h3>${escapeHtml(g.nom)}</h3></div>
...
function renderTable(crops) {
  $('mcViewTable').innerHTML = `
    <div class="card table-wrap">
      <table class="data-table">
        <thead>
          <tr>
            <th>Cultiu</th><th>Família</th><th>SI</th><th>SE</th><th>TR</th><th>CO</th><th>Etiquetes</th><th class="${mcState.showNotes ? '' : 'hidden'}">Notes</th>
          </tr>
        </thead>
        <tbody>
          ${crops.map(c => `
            <tr>
              <td><b>${escapeHtml(c.nom)}</b><div class="small">${escapeHtml(c.tipus_implantacio)}</div></td>
              <td>${escapeHtml(MC_FAMILY_MAP[c.familia].nom)}</td>
              ...
              <td>${c.tags.join(', ') || '—'}</td>
              <td class="${mcState.showNotes ? '' : 'hidden'}">${escapeHtml(c.notes)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  $('mcViewGantt').innerHTML = `
    <div class="card gantt-wrap">
      <div class="gantt">
        ...
        ${crops.map(c => `
          <div class="g-row">
            <div class="g-crop">${escapeHtml(c.nom)}<span class="small">${escapeHtml(MC_FAMILY_MAP[c.familia].nom)}</span></div>
...
function renderMobile(crops) {
  $('mcViewMobile').innerHTML = `
    <div class="mobile-list">
      ${crops.map(c => `
        <article class="card mobile-card">
          <div class="mobile-head">
            <div>
              <div class="crop-name">${escapeHtml(c.nom)}</div>
              <div class="crop-sub">${escapeHtml(MC_FAMILY_MAP[c.familia].nom)}</div>
            </div>
          ...
          <div class="crop-notes ${mcState.showNotes ? '' : 'hidden'}" style="margin-top:10px">${escapeHtml(c.notes)}</div>
        </article>
      `).join('')}
    </div>
  `;
...
function render() {
  ...
  $(`mcView${mcState.view.charAt(0).toUpperCase()}${mcState.view.slice(1)}`).classList.add('active');
  $$('[data-mc-view-btn]').forEach(btn => btn.classList.toggle('active', btn.dataset.mcViewBtn === mcState.view));
}

function bind() {
  $('mcQ').addEventListener('input', (e) => { mcState.q = e.target.value.trim(); render(); });
  $('mcFamily').addEventListener('change', (e) => { mcState.family = e.target.value; render(); });
  $('mcTag').addEventListener('change', (e) => { mcState.tag = e.target.value; render(); });
  $('mcPhase').addEventListener('change', (e) => { mcState.phase = e.target.value; render(); });
  $('mcHalf').addEventListener('change', (e) => { mcState.half = e.target.value; render(); });
  $('mcClear').addEventListener('click', () => {
    mcState.q = ''; mcState.family = 'all'; mcState.tag = 'all'; mcState.phase = 'all'; mcState.half = 'all';
    $('mcQ').value = ''; $('mcFamily').value = 'all'; $('mcTag').value = 'all'; $('mcPhase').value = 'all'; $('mcHalf').value = 'all';
    render();
  });
  $('mcToggleNotes').addEventListener('click', () => {
    mcState.showNotes = !mcState.showNotes;
    $('mcToggleNotes').textContent = mcState.showNotes ? 'Amaga notes' : 'Mostra notes';
    render();
  });
  $$('[data-mc-view-btn]').forEach(btn => btn.addEventListener('click', () => {
...
export function initCalendariPage() {
  if (!$('mcMetaPills')) return;

  // correcció d'una propietat mal escrita si s'edita el dataset a mà
  CALENDARI_DATA.crops.forEach(c => {
    if (c.calendaris && c.calendaris.sembla_exterior && !c.calendaris.sembra_exterior) {
      c.calendaris.sembra_exterior = c.calendaris.sembla_exterior;
      delete c.calendaris.sembla_exterior;
    }
  });
