...
(function(){
  let _inited = false;
  // capture the real init function (do NOT overwrite it)
  const _origInit = (typeof initCalendari === "function") ? initCalendari : null;

  function safeInit(){
    if(_inited) return;
    try{
      if(document.getElementById("mcQ") && _origInit){
        _origInit();
        _inited = true;
      }
    }catch(e){
  ...
  // Expose manual hooks for debugging
