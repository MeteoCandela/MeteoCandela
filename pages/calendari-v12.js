...
// --- Auto-init v12 (module-safe + debug) ---
(function(){
  let _inited = false;

  // Expose initCalendari to window even when this file is imported as an ES module.
  if (typeof initCalendari === "function") {
    window.initCalendari = initCalendari;
  }

  const _origInit = window.initCalendari;

  function safeInit(){
    if(_inited) return;
    try{
      const hasDom = !!document.getElementById("mcQ");
      if(!hasDom) return;

      if (typeof _origInit !== "function") {
        console.error("[calendari] initCalendari is not a function (not exposed)");
        return;
      }
      ...
      // Post-check: did we render anything?
      const cards = document.querySelectorAll("#mcViewCards .mc-card, #mcViewMobile .mc-card").length;
      if (cards === 0) {
        console.warn("[calendari] init ran but rendered 0 cards. Check console for earlier errors.");
      }
    }catch(e){
  ...
  // Debug hooks
  window.calendariInit = safeInit;
  window.calendariInitRaw = _origInit;
